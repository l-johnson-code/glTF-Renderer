...
class Bloom {
    ...
    void Create(Gpu::Resources* resources, uint16_t width, uint16_t height, uint8_t max_iterations);
    void Resize(uint16_t width, uint16_t height, uint8_t max_iterations);
    void Execute(CommandContext* context, Gpu::Texture* input, D3D12_RESOURCE_STATES input_resource_states, uint8_t iterations, float strength);
