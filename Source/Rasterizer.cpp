...
void Rasterizer::DrawScene(CommandContext* context, const Settings* settings, const ExecuteParams* execute_params)
{
	...
	ForwardPass::Config config = {
		...
		.lights = execute_params->gpu_scene->LightBuffer().Resource()->GetGPUVirtualAddress(),
		.materials = execute_params->gpu_scene->MaterialBuffer().Resource()->GetGPUVirtualAddress(),
